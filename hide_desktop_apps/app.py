...
--------
  ...
  Ctrl+Alt+T  (configurable) → toggle taskbar
...
Config  (~AppData\\HideDesktopApps\\config.ini — auto-created on first run)
------
  [hotkey]
...
# ---------------------------------------------------------------------------
# Paths  —  config and icon live in %APPDATA%\HideDesktopApps
# ---------------------------------------------------------------------------

_APP_DIR    = os.path.join(os.environ.get("APPDATA", os.path.expanduser("~")), "HideDesktopApps")
os.makedirs(_APP_DIR, exist_ok=True)
CONFIG_FILE = os.path.join(_APP_DIR, "config.ini")
ICON_FILE   = os.path.join(_APP_DIR, "hide_desktop.ico")
...
# ---------------------------------------------------------------------------
# .ico file  —  written to %APPDATA%\HideDesktopApps\ on first run
# ---------------------------------------------------------------------------
...
def _startup_target() -> tuple[str, str]:
    """Return (pythonw_path, args) for the VBS startup launcher.

    When installed via pip the cleanest launcher is::

        pythonw.exe -m hide_desktop_apps

    This always uses the same Python environment the package is installed in.
    """
    pythonw = os.path.join(os.path.dirname(sys.executable), "pythonw.exe")
    if not os.path.isfile(pythonw):
        pythonw = sys.executable
    return pythonw, "-m hide_desktop_apps"
...
def _write_vbs(vbs_path: str, pythonw: str, args: str,
               delay_s: int = 30) -> None:
    """Write the VBS launcher.

    WindowStyle 0 = hidden window; False = fire-and-forget.
    """
    q          = chr(34)
...
def _add_to_startup(cfg: configparser.ConfigParser | None = None) -> None:
    """Write HideDesktopApps.vbs to the Startup folder."""
    if cfg is None:
        cfg = _load_config()
    _scrub_old_startup_files()
    pythonw, args = _startup_target()
    delay_s = max(0, cfg["startup"].getint("delay", 30))
    vbs = _get_vbs_path()
    try:
        os.makedirs(_STARTUP_DIR, exist_ok=True)
        _write_vbs(vbs, pythonw, args, delay_s)
...
# ---------------------------------------------------------------------------
# main()  —  one run of the app (restart loop lives in __main__.py)
# ---------------------------------------------------------------------------
